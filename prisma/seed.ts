...
async function main() {
  ...
  const product = await prisma.product.upsert({
    ...
    create: {
      ...
      images: {
        create: [
          { url: '/products/plain_salted.png', altText: 'Plain Salted', sortOrder: 0 },
          { url: '/products/peri_peri.png', altText: 'Peri Peri', sortOrder: 1 },
  ...
  for (const flavour of flavours) {
    for (const size of sizes) {
      const basePrice =
        size.weightGrams === 50 ? 60 :
        size.weightGrams === 100 ? 110 : 260
